...
#define Button_Event(handler, event_)              \
	...
	{                                                \
		...
		if (handler->callback_fun[event_] != NULL) \
			handler->callback_fun[event_](handler);  \
	} while (0)
...
void Button_Register(Button_t *button, Button_Event_t event, Button_Callback_t callback_fun)
{
	...
	button->callback_fun[event] = (void (*)(void *))callback_fun;
}
...
#error Button in the sampling time is less than 1.(button.c)
	...
	if (button->press_count >= Button_Sampling_Window)
	{
		...
		case 1:
			// 产生按键按下事件
			Button_Event(button, Press_Hold);
	...
	else
	{
		...
		case 0:
			...
			// 产生空闲事件
			Button_Event(button, Press_None);
			break;
		case 1:
