 ...
 * <tr><td>2024-03-10     <td>1.1         <td>SY7_yellow  <td>重构了SEML库的初始化代码
 * </table>
 *@details :
 * ============================================================================
 ...
 * 个参数,使用SEML库的GPIO可以调用SEML_Button_Init传入端口号和引脚号)
 * 2. 调用Button_Register注册按键事件回调函数
 * 3. 将Button_Scan放入毫秒级定时器中断中(10ms左右)
 ...
 * @param button 按键句柄
 */
#define Get_Button_Event(button) ((button)->event)

#ifdef USE_SEML_LIB
/**
 * @brief SEML库按键初始化
 * @param button 按键句柄
 * @param GPIO_Part GPIO端口号
 * @param GPIO_Pin GPIO引脚号
 * @param active_level 按键按下电平
 */
#define SEML_Button_Init(button, active_level, GPIO_Part, GPIO_Pin)                                      \
	do                                                                                                     \
	{                                                                                                      \
		static GPIO_Handle_t KEY_##GPIO_Part##GPIO_Pin##_Handle;                                             \
		SEML_GPIO_Pin_Register(&(KEY_##GPIO_Part##GPIO_Pin##_Handle), GPIO_Part, GPIO_Pin);                  \
		Button_Init(button, (FunctionalState)active_level,                                                   \
								(uint8_t(*)(void *))SEML_GPIO_Pin_Input, (void *)&(KEY_##GPIO_Part##GPIO_Pin##_Handle)); \
	} while (0)
