...
static SEML_StatusTypeDef queue_reapply(s_queue *queue)
{
	...
	// 申请空间
	new_queue = realloc(queue->address, (queue->size + reapply_size) * queue->elem_size);
	// 检查是否申请成功
	if (new_queue == NULL)
	{
		queue->error_code = queue_reapply_error;
	...
	if (queue->front > queue->rear)
	{
		if (queue->rear < reapply_size)
		{
			...
			memcpy((uint8_t *)queue->address + last_size * queue->elem_size, (uint8_t *)queue->address, queue->elem_size * queue->rear);
			// 队尾移动到新位置
		...
		else
		{
			...
			{
				if (i < reapply_size)
					memcpy((uint8_t *)queue->address + (last_size + i) * queue->elem_size, (uint8_t *)queue->address + i * queue->elem_size, queue->elem_size);
				else
					memcpy((uint8_t *)queue->address + (last_size + i - reapply_size) * queue->elem_size, (uint8_t *)queue->address + i * queue->elem_size, queue->elem_size);
			}
			// 大于拓展部份前移
			if (queue->rear != 0)
...
SEML_StatusTypeDef EnQueue(s_queue *queue, const void *data)
{
	...
	// 入队
	memcpy((uint8_t *)queue->address + queue->rear * queue->elem_size, data, queue->elem_size);
	queue->rear += 1;
...
SEML_StatusTypeDef DeQueue(s_queue *queue, void *data)
{
	...
	// 出队
	memcpy(data, (uint8_t *)queue->address + queue->front * queue->elem_size, queue->elem_size);
	queue->front += 1;
...
SEML_StatusTypeDef DeQueueRear(s_queue *queue, void *data)
{
	...
	// 出队
	memcpy(data, (uint8_t *)queue->address + queue->rear * queue->elem_size, queue->elem_size);
	__SEML_UNLOCK(queue);
	return SEML_OK;
}
...
SEML_StatusTypeDef GetQueueRear(s_queue *queue, void *data)
{
	...
	// 读取队头数据
	memcpy(data, (uint8_t *)queue->address + temp * queue->elem_size, queue->elem_size);
	__SEML_UNLOCK(queue);
	return SEML_OK;
}
...
SEML_StatusTypeDef VisitQueueElem(s_queue *queue, uint16_t index, void *data, queue_read_while_t status)
{
	...
	case queue_read:
		...
		break;
	case queue_write:
		memcpy((uint8_t *)queue->address + temp * queue->elem_size, data, queue->elem_size);
		break;
	}
	__SEML_UNLOCK(queue);
	return SEML_OK;
